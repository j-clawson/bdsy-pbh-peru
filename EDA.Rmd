...
```{r}
df_nation <- peru[c("year","week", "under_5_death", "over_60_death", "under_5_case")]
...
vaccine_dates <- tibble(
  date    = c(pcv7_intro_date, pcv10_intro_date, pcv13_intro_date),
...
p2 <- 
  ggplot(df_nation, aes(x = date, y = over60)) +
    geom_line() +
    geom_vline( data = vaccine_dates, aes(xintercept = date, colour = vaccine, linetype = vaccine), size = 0.8) +
    scale_colour_brewer( name = "Vaccine introduction", palette = "Dark2") +
    scale_linetype_discrete(name = "Vaccine introduction") +
...
deaths_by_region <- peru %>%
  group_by(region) %>%
  summarise(total_deaths = sum(under_5_death),
            total_cases = sum(under_5_case),
            .groups = "drop")
...
```{r}
# 1. Define region groups
high_wealth   <- c("LIMA", "MOQUEGUA", "ICA", "AREQUIPA", "TACNA", "MADRE DE DIOS", "CUSCO", "PASCO")
moderate_wealth <- c("ANCASH", "LA LIBERTAD", "TUMBES", "PIURA", "JUNIN", "LAMBAYEQUE", "UCAYALI", "LORETO")
low_wealth    <- c("AYACUCHO", "CAJAMARCA", "AMAZONAS", "PUNO", "HUANCAVELICA", "SAN MARTIN", "APURIMAC", "HUANUCO")

# 2. Add GDP category to df_plot
df_plot <- deaths_by_region %>%
  filter(region != "CALLAO") %>%
  mutate(
    death_rate = (total_deaths / total_cases) * 1000,
    region     = factor(region, levels = gdp_pc_order[gdp_pc_order != "CALLAO"]),
    gdp_cat = case_when(
      region %in% high_wealth      ~ "High",
      region %in% moderate_wealth  ~ "Moderate",
      region %in% low_wealth       ~ "Low",
      TRUE                        ~ NA_character_
    ),
    gdp_cat = factor(gdp_cat, levels = c("High", "Moderate", "Low"))
  )

# 3. Draw the colored bar chart
line_df <- data.frame(line_label = "National Average", y = peru_mortality)

p3 <- ggplot(df_plot, aes(x = region, y = death_rate, fill = gdp_cat)) +
  geom_col() +
  geom_hline(
    data = line_df,
    aes(yintercept = y, color = line_label),
    linetype = "dashed",
    size = 1,
    show.legend = TRUE
  ) +
  scale_fill_manual(
    values = c("High" = "#D62728", "Moderate" = "#FF7F0E", "Low" = "#1F77B4"),
    name = "GDP per Capita"
  ) +
  scale_color_manual(
    values = c("National Average" = "black"),
    name = NULL
  ) +
  guides(
    fill = guide_legend(order = 1, override.aes = list(linetype = 0, color = NA)),
    color = guide_legend(order = 2)
  ) +
  labs(
    title = "<5 yo Average Mortality Rate Per Thousand by Region (2000-2023)",
    x     = "Region (ordered by GDP per capita, richest→poorest)",
    y     = "Mortality rate per thousand"
  ) +
  theme_minimal() +
  theme(
...
```{r}
ggsave(
  filename = "Regions_mortality_rate.png",
  plot     = p3,
  width    = 7,
  height   = 3,
  units    = "in",
...
region_to_point <- function(data, region_name) {
  ...
  p <- ggplot(data, aes(x = date, y = under5_deaths)) +
    geom_line() + 
    geom_vline(data = vaccine_dates, aes(xintercept = date, colour = vaccine, linetype = vaccine), size = 0.8) +
    scale_colour_brewer(name = "Vaccine Introduction", palette = "Dark2") +
    scale_linetype_discrete(name = "Vaccine Introduction") +
...
```{r}
p4a <- region_to_point(df_region, "SAN MARTIN")
p4b <- region_to_point(df_region, "PUNO")
