...
```{r}
gdp_pc_order <- c(
  "LIMA", "MOQUEGUA", "ICA", "AREQUIPA", "TACNA",
  "MADRE DE DIOS", "CUSCO", "PASCO", "ANCASH", "LA LIBERTAD",
  "TUMBES", "PIURA", "JUNIN", "LAMBAYEQUE", "UCAYALI",
  "LORETO", "AYACUCHO", "CAJAMARCA", "AMAZONAS", "PUNO",
  "HUANCAVELICA", "SAN MARTIN", "APURIMAC", "HUANUCO"
)

# 2) Compute death rate and re‐factor region
...
p3 <- ggplot(df_plot, aes(x = region, y = death_rate)) +
  geom_col(fill = "#fb8500") +
  labs(
    title = "<5 Mortality Rate by Region",
    x     = "Region (ordered by GDP per capita, richest→poorest)",
    y     = "Mortality rate (deaths ÷ cases)"
  ) +
  theme_minimal() +
  theme(
